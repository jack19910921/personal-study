...
```mermaid
flowchart TD
    subgraph Accuracy["AIME 准确率趋势"]
        direction LR
        A1[初始 15.6%] --> A2[显著跃升至 77.9%]
        A2 --> A3[Cons@16 达 86.7%]
        A3 --> A4[超越人类竞赛平均水平]
    end

    subgraph Length["平均响应长度趋势"]
        direction LR
        L1[初始短回复] --> L2[稳定增长]
        L2 --> L3[数百至数千 tokens]
        L3 --> L4[8.2k step 处显著跳跃]
    end
...
flowchart LR
    ...
    subgraph GRPO["GRPO (DeepSeek)"]
        ...
        GLM --> GResp["生成 G 个回复 {o1...oG}"]
        GResp --> GReward[规则/模型奖励]
        GReward --> GNorm["组内归一化<br/>Ai = (ri - μ) / σ"]
