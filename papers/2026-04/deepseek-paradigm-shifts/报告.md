...
#### 传统 LLM vs 推理 LLM vs DeepSeek 范式

```mermaid
flowchart TD
    subgraph 传统LLM
        A1[预训练: 自监督 next-token prediction] --> A2[后训练: SFT + RLHF 对齐]
        A2 --> A3[推理: 单步自回归生成]
    end

    subgraph 推理LLM
        B1[预训练: 含推理专用数据的自监督] --> B2[后训练: PRM 过程奖励 + MCTS + Self-Correction]
        B2 --> B3[推理: 多步 CoT + 束搜索 / MCTS 路径搜索]
    end

    subgraph DeepSeek范式
        C1[预训练: MoE + MLA + MTP + FP8 混合精度] --> C2[后训练: GRPO 纯RL 或 SFT+RL 双路径]
        C2 --> C3[推理: 压缩 KV cache + MoE 稀疏激活 37B/671B]
    end
...
flowchart LR
    subgraph PPO["PPO (传统 RLHF)"]
        direction LR
        PQ[问题 q] --> PLM[LLM 策略模型 πθ]
        PLM --> PR[生成回复 o]
        PR --> PRM[奖励模型 / 价值模型 Vφ]
        PRM --> PPPO[PPO 优化]
        PRef[参考模型 π_ref] -.KL 惩罚.-> PPPO
        PPPO --> PUpdate[更新 πθ]
    end

    subgraph GRPO["GRPO (DeepSeek)"]
        direction LR
        GQ[问题 q] --> GLM[LLM 策略模型 πθ_old]
        GLM --> GResp["生成 G 个回复 {o1, o2, ..., oG}"]
        GResp --> GReward["规则奖励: 准确率 + 格式"]
        GReward --> GNorm["组内归一化: Ai = (ri - μ) / σ"]
...
| **优势计算** | 基于价值模型的 TD 误差 | **组内归一化: (ri - mean) / std** |
| **KL 惩罚** | 加到 Reward 中 | **直接加到 Loss 函数** |
| **采样数** | 通常 1 个回复 | **每组 G 个回复** |
